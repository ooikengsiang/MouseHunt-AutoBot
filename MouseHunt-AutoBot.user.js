...
// ################################################################################################
//   Most Wanted Reward - Start
// ################################################################################################

function checkMostWantedReward()
{
	// check for any unclaimed reward
	var claimRewardLinks = document.getElementsByClassName('townOfGnawniaHUD-actionButton claim active');
	if (claimRewardLinks)
	{
		// claim the reward first
		fireEvent(claimRewardLinks[0], 'click');
		
		// after claiming the reward, we need to come back to check for reward again
		window.setTimeout(function () { checkMostWantedReward() }, 30000);
	}
	claimRewardLinks = null;
	
	// reveal any unreveal most wanted
	var revealRewardLinks = document.getElementsByClassName('townOfGnawniaHUD-actionButton reveal active');
	if (revealRewardLinks)
	{
		// reveal the reward and most wanted
		fireEvent(revealRewardLinks[0], 'click');
	}
	revealRewardLinks = null;
}

// ################################################################################################
//   Most Wanted Reward - End
// ################################################################################################
