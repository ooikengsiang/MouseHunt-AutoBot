...
// @include		*://hi5.com/friend/games/MouseHunt*
// @include		*://mousehunt.hi5.hitgrab.com/*
...
// // Play sound when the bait / cheese quantity reach zero. (true/false)
var isBaitEmptyWarningSound = false;
...
// // Do not play any warning sound between the defined time in doNotDistrubStart and doNotDistrubEnd.  (true/false)
// // Note: It will override the isBaitEmptyWarningSound / isKingWarningSound settings.
var isDoNotDistrub = false;

// // Time between where the bot shouldn't play any warning sound. (24 hour time format)
// // Note: It only take effect if isDoNotDistrub = true;
// // Example: 2330 - 600, which mean do not distrub between 11:30pm to 06:00am
var doNotDistrubStart = 2330;
var doNotDistrubEnd = 600;
...
{
    retrieveDataNextHuntTime();
    retrieveDataBaitQuantity();
    retrieveDataKingReward();

    // get trap check time
    if (enableTrapCheck)
    {
        retrieveDataCheckTime();
    }
}

function retrieveDataNextHuntTime()
{
    var huntTimerElement = null;
    var headerElement = document.getElementById('envHeaderImg');
    if (headerElement)
    {
        var divElements = headerElement.getElementsByTagName('div');
        for (var i = 0; i < divElements.length; ++i)
        {
            var tempDivElement = divElements[i];
            var divClass = tempDivElement.getAttribute('class');
            if (divClass && divClass.indexOf('huntersHornView__countdown') != -1)
            {
                huntTimerElement = tempDivElement;
                break;
            }
        }
    }
    ...
    // huntTimer
    if (huntTimerElement && huntTimerElement.innerText)
    {
        var huntTimeStr = huntTimerElement.innerText;
        var midIndex = huntTimeStr.indexOf(":");
        var minuteStr = huntTimeStr.substring(0, midIndex);
        var secondStr = huntTimeStr.substring(midIndex + 1);

        var minute = parseInt(minuteStr);
        var second = parseInt(secondStr);
        if (!isNaN(minute) && !isNaN(second))
        {
            nextActiveTime = minute * 60 + second;

            // if next hunt time is excatly 1 second away, it is hihgly possible we encounter a king rewards
            if (nextActiveTime == 1)
            {
        ...
        {
            // no valid number, maybe it is hunt time
            hornTime = 0;
        }
    }
    else
    {
        // fail to retrieve next hunt time
        // reload the whole page
        reloadPage(false);
    }
}

function retrieveDataCheckTime()
{
    var nowDate = new Date();
    checkTimeDelay = checkTimeDelayMin + Math.round(Math.random() * (checkTimeDelayMax - checkTimeDelayMin));
    checkTime = (nowDate.getMinutes() >= trapCheckTimeDiff) ? 3600 + (trapCheckTimeDiff * 60) - (nowDate.getMinutes() * 60 + nowDate.getSeconds()) : (trapCheckTimeDiff * 60) - (nowDate.getMinutes() * 60 + nowDate.getSeconds());
    checkTime += checkTimeDelay;
    nowDate = undefined;
}

function retrieveDataBaitQuantity()
{
    // retrieve the cheese quantity
    var baitQuantityElement = document.getElementById('hud_baitQuantity');
    if (baitQuantityElement)
    {
        var baitQauntityTemp = parseInt(baitQuantityElement.innerText);
        if (!isNaN(baitQauntityTemp))
        {
            baitQuantity = baitQauntityTemp;
        }
    }
    baitQuantityElement = undefined;
}

function retrieveDataKingReward()
{
    ...
    if (!foundKingReward)
    {
        var kingRewardElements = document.getElementsByClassName('mousehuntPage-puzzle active');
        if (kingRewardElements && kingRewardElements.length > 0)
        {
            foundKingReward = true;
        }
        kingRewardElements = undefined;
    }

    // update the global variable
    isKingReward = foundKingReward;
}
...
function baitZeroAction()
{
    if (baitQuantity == 0)
    {
        ...
        if (isBaitEmptyWarningSound)
        {
            playWarningSound();
        }

        // wait until the hunting is resumed
        waitHuntingResume();
    }
}
...
function countdownTimer()
{
    ...
    else if (baitQuantity == 0)
    {
        ...
        // pause the script
        window.setTimeout(function () { (waitHuntingResume)() }, timerRefreshInterval * 1000);
    }
    else
    {
        ...
        else
        {
            if (enableTrapCheck)
            {
                if (checkTime <= 0)
                {
                    // check on the trap by refresh the camp
                    gotoCamp(true);

                    // reload the next trap check time
                    retrieveDataCheckTime();
                }
...
function waitHuntingResume()
{
    // check if user already resolve all the problem that prevent us from hunting
    var isResumeHunting = true;

    // check if bait is more than zero
    if (baitQuantity <= 0)
    {
        retrieveDataBaitQuantity();
        if (baitQuantity <= 0)
        {
            isResumeHunting = false;
        }
    }

    // check if king reward is resolved by user
    if (isKingReward)
    {
        retrieveDataKingReward();
        if (isKingReward)
    ...
    if (isResumeHunting)
    {
        // make sure all warning sound is stopped
        stopWarningSound();
    ...
    {
        // wait to check everything again
        window.setTimeout(function () { (waitHuntingResume)() }, timerRefreshInterval * 1000);
    }
}
...
function embedTimer(targetPage)
{
    if (showTimerInPage)
    {
        ...
        if (!headerElement)
        {
            headerElement = document.getElementById('maintenanceHeader');
        }
        ...
        if (headerElement)
        {
            ...
            // Aggressive mode + Horn Delay Min + Horn Delay Max
            ...
            if (enableTrapCheck)
                ...
                preferenceHTMLStr += '<input type="text" id="TrapCheckTimeOffsetInput" name="TrapCheckTimeOffsetInput" disabled="disabled" value="' + trapCheckTimeDiff.toString() + '"/> minutes';
            preferenceHTMLStr += '</td>';
            preferenceHTMLStr += '</tr>';
            preferenceHTMLStr += '<tr>';
            preferenceHTMLStr += '<td style="height:24px; text-align:right;">';
            ...
            preferenceHTMLStr += '<tr>';
            preferenceHTMLStr += '<td style="height:24px; text-align:right;">';
            preferenceHTMLStr += '<a title="Play sound when no bait / cheese"><b>Bait Empty Sound</b></a>';
            preferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
            preferenceHTMLStr += '</td>';
            preferenceHTMLStr += '<td style="height:24px">';
            if (isKingWarningSound)
                preferenceHTMLStr += '<input type="radio" id="PlayBaitEmptySoundInputTrue" name="PlayBaitEmptySoundInput" value="true" checked="checked"/> True';
            else
                preferenceHTMLStr += '<input type="radio" id="PlayBaitEmptySoundInputTrue" name="PlayBaitEmptySoundInput" value="true" /> True';
            preferenceHTMLStr += '   ';
            if (isKingWarningSound)
                preferenceHTMLStr += '<input type="radio" id="PlayBaitEmptySoundInputFalse" name="PlayBaitEmptySoundInput" value="false" /> False';
            else
                preferenceHTMLStr += '<input type="radio" id="PlayBaitEmptySoundInputFalse" name="PlayBaitEmptySoundInput" value="false" checked="checked"/> False';
            preferenceHTMLStr += '</td>';
            preferenceHTMLStr += '</tr>';
            ...
            // King Reward Reload + Reload Interval
            preferenceHTMLStr += '<tr>';
            preferenceHTMLStr += '<td style="height:24px; text-align:right;">';
            preferenceHTMLStr += '<a title="Reload the the page according to King Reward Resume Time when encount King Reward"><b>King Reward Reload</b></a>';
            preferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
            preferenceHTMLStr += '</td>';
            preferenceHTMLStr += '<td style="height:24px">';
            if (reloadKingReward)
            ...
            preferenceHTMLStr += '<tr>';
            preferenceHTMLStr += '<td style="height:24px; text-align:right;">';
            preferenceHTMLStr += '<a title="Duration of pausing the script before reload the King\'s Reward page (in seconds)"><b>King Reward Reload Time</b></a>';
            preferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
            preferenceHTMLStr += '</td>';
            preferenceHTMLStr += '<td style="height:24px">';
            if (reloadKingReward)
            ...
            // Do not distrub + time do not distrub
            preferenceHTMLStr += '<tr>';
            preferenceHTMLStr += '<td style="height:24px; text-align:right;">';
            preferenceHTMLStr += '<a title="Mute all warning sound during the defined time."><b>Do Not Distrub</b></a>';
            preferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
            preferenceHTMLStr += '</td>';
            preferenceHTMLStr += '<td style="height:24px">';
            if (isDoNotDistrub)
                preferenceHTMLStr += '<input type="radio" id="DoNotDistrubInputTrue" name="DoNotDistrubInput" value="true" onchange="if (document.getElementById(\'DoNotDistrubInputTrue\').checked == true) { document.getElementById(\'DoNotDistrubStartInput\').disabled=\'\'; document.getElementById(\'DoNotDistrubEndInput\').disabled=\'\';}" checked="checked"/> True';
            else
                preferenceHTMLStr += '<input type="radio" id="DoNotDistrubInputTrue" name="DoNotDistrubInput" value="true" onchange="if (document.getElementById(\'DoNotDistrubInputTrue\').checked == true) { document.getElementById(\'DoNotDistrubStartInput\').disabled=\'\'; document.getElementById(\'DoNotDistrubEndInput\').disabled=\'\';}"/> True';
            preferenceHTMLStr += '   ';
            if (isDoNotDistrub)
                preferenceHTMLStr += '<input type="radio" id="DoNotDistrubInputFalse" name="DoNotDistrubInput" value="false" onchange="if (document.getElementById(\'DoNotDistrubInputFalse\').checked == true) { document.getElementById(\'DoNotDistrubStartInput\').disabled=\'disabled\'; document.getElementById(\'DoNotDistrubEndInput\').disabled=\'disabled\';}"/> False';
            else
                preferenceHTMLStr += '<input type="radio" id="DoNotDistrubInputFalse" name="DoNotDistrubInput" value="false" onchange="if (document.getElementById(\'DoNotDistrubInputFalse\').checked == true) { document.getElementById(\'DoNotDistrubStartInput\').disabled=\'disabled\'; document.getElementById(\'DoNotDistrubEndInput\').disabled=\'disabled\';}" checked="checked"/> False';
            preferenceHTMLStr += '</td>';
            preferenceHTMLStr += '</tr>';
            preferenceHTMLStr += '<tr>';
            preferenceHTMLStr += '<td style="height:24px; text-align:right;">';
            preferenceHTMLStr += '<a title="Define the time between where all warning sound will be overrided (in 24 hour time format, example 23:30)"><b>Do Not Distrub Time</b></a>';
            preferenceHTMLStr += '&nbsp;&nbsp;:&nbsp;&nbsp;';
            preferenceHTMLStr += '</td>';
            preferenceHTMLStr += '<td style="height:24px">';
            if (isDoNotDistrub)
                preferenceHTMLStr += '<input type="text" id="DoNotDistrubStartInput" name="DoNotDistrubStartInput" value="' + doNotDistrubStart.toString() + '"/> start';
            else
                preferenceHTMLStr += '<input type="text" id="DoNotDistrubStartInput" name="DoNotDistrubStartInput" disabled="disabled" value="' + doNotDistrubStart.toString() + '"/> start';
            preferenceHTMLStr += ' ~ ';
            if (isDoNotDistrub)
                preferenceHTMLStr += '<input type="text" id="DoNotDistrubEndInput" name="DoNotDistrubEndInput" value="' + doNotDistrubEnd.toString() + '"/> end';
            else
                preferenceHTMLStr += '<input type="text" id="DoNotDistrubEndInput" name="DoNotDistrubEndInput" disabled="disabled" value="' + doNotDistrubEnd.toString() + '"/> end';
            preferenceHTMLStr += '</td>';
            preferenceHTMLStr += '</tr>';

            // Pause at invalid location
            preferenceHTMLStr += '<tr>';
            preferenceHTMLStr += '<td style="height:24px; text-align:right;">';
            ...
            preferenceHTMLStr += '<input type="button" id="PreferenceSaveInput" value="Save" onclick="	\
                ...
                if (document.getElementById(\'PlayBaitEmptySoundInputTrue\').checked == true) { window.localStorage.setItem(\'PlayBaitEmptySound\', \'true\'); } else { window.localStorage.setItem(\'PlayBaitEmptySound\', \'false\'); }	\
                ...
                if (document.getElementById(\'DoNotDistrubInputTrue\').checked == true) { window.localStorage.setItem(\'IsDoNotDistrub\', \'true\'); } else { window.localStorage.setItem(\'IsDoNotDistrub\', \'false\'); }	\
                window.localStorage.setItem(\'DoNotDistrubStart\', document.getElementById(\'DoNotDistrubStartInput\').value); \
                window.localStorage.setItem(\'DoNotDistrubEnd\', document.getElementById(\'DoNotDistrubEndInput\').value);	\
...
function loadPreferenceSettingFromStorage()
{
    ...
    var playBaitEmptySoundTemp = getStorage("PlayBaitEmptySound");
    if (playBaitEmptySoundTemp == undefined || playBaitEmptySoundTemp == null)
    {
        setStorage("PlayKingRewardSound", isBaitEmptyWarningSound.toString());
    }
    else if (playBaitEmptySoundTemp == true || playBaitEmptySoundTemp.toLowerCase() == "true")
    {
        isBaitEmptyWarningSound = true;
    }
    else
    {
        isBaitEmptyWarningSound = false;
    }
    playBaitEmptySoundTemp = undefined;
    ...
    // Do not distrub
    var isDoNotDistrubTemp = getStorage("IsDoNotDistrub");
    if (isDoNotDistrubTemp == undefined || isDoNotDistrubTemp == null)
    {
        setStorage("IsDoNotDistrub", isDoNotDistrub.toString());
    }
    else if (isDoNotDistrubTemp == true || isDoNotDistrubTemp.toLowerCase() == "true")
    {
        isDoNotDistrub = true;
    }
    else
    {
        isDoNotDistrub = false;
    }
    isDoNotDistrubTemp = undefined;

    var doNotDistrubStartTemp = getStorage("DoNotDistrubStart");
    var doNotDistrubEndTemp = getStorage("DoNotDistrubEnd");
    if (doNotDistrubStartTemp == undefined || doNotDistrubStartTemp == null || doNotDistrubEndTemp == undefined || doNotDistrubEndTemp == null)
    {
        setStorage("DoNotDistrubStart", doNotDistrubStart);
        setStorage("DoNotDistrubEnd", doNotDistrubEnd);
    }
    else
    {
        doNotDistrubStart = parseInt(doNotDistrubStartTemp);
        doNotDistrubEnd = parseInt(doNotDistrubEndTemp);
    }
    doNotDistrubStartTemp = undefined;
    doNotDistrubEndTemp = undefined;
...
function afterSoundingHorn()
{
    ...
    {
        var hornElement = null;
        var aElements = headerElement.getElementsByTagName('a');
        for (var i = 0; i < aElements.length; ++i)
        {
            var tempAElement = aElements[i];
            var title = tempAElement.getAttribute('title');
            if (title && title.indexOf('Horn') != -1)
            {
                hornElement = tempAElement;
                break;
            }
        }
        ...
        {
            var tempDivElement = divElements[i];
            var className = tempDivElement.getAttribute('class');
            if (className && className.indexOf('messageText') != -1)
            {
                messageElement = tempDivElement;
                break;
            }
        }
        ...
        if (hornElement)
        {
            var hornStatus = hornElement.getAttribute('class');
            if (hornStatus && 
                hornStatus.indexOf("ready") != -1 && 
                hornStatus.indexOf("reveal") != -1)
            {
                // horn still there for some reason

                // check if we already sounded the horn
                if (messageElement && messageElement.innerText)
                {
                    if (messageElement.innerText.indexOf("You sounded your horn") == 0)
                    {
                        // user already sounded his / her horn
                        // maybe on another device
                        isReload = false;
                    }
                    else if (messageElement.innerText.indexOf("King's Reward") != -1)
                    {
                        // King's reward
                        isKingReward = true;
                        isReload = false;
    ...
    {
        // update timer
        displayTimer("Reloading turn...", "Reloading turn...", "Reloading turn...");

        // sound horn might be broken, use URL instead
        reloadPage(true);
    }
    else
    {
...
function kingRewardAction()
{
    ...
    // remember last king's reward time
    var nowDate = new Date();
    ...
    // check and see if user resolved the king reward or not
...
function kingRewardCountdownTimer()
{
    ...
    // check if king reward still there
    retrieveDataKingReward();
    if (!isKingReward)
    {
        // king reward resolved by user manually
        waitHuntingResume();
        return;
    }
    ...
    if (reloadKingReward)
    {
        // calculate the king reward resume timer countdown
        kingPauseTime -= intervalTime;

        // calculate how long we have waited
        lastKingRewardSumTime += intervalTime;
    ...
    // call itself to check again later
    window.setTimeout(function () { (kingRewardCountdownTimer)() }, timerRefreshInterval * 1000);
...
// ################################################################################################
//   Warning Sound Function - Start
// ################################################################################################

function playWarningSound()
{
    // check if play warning sound is allowed during this time
    if (isDoNotDistrub)
    {
        var nowDate = new Date();
        var nowTime = nowDate.getHours() * 100 + nowDate.getMinutes();

        if (doNotDistrubStart >= doNotDistrubEnd)
        {
            // include pass mid night time
            if ((nowTime >= doNotDistrubStart && nowTime < 2400) ||
                (nowTime >= 0 && nowTime <= doNotDistrubEnd))
            {
                // now is do not distrub time, so no warning sound
                return;
            }
        }
        else
        {
            if (nowTime >= doNotDistrubStart && nowTime <= doNotDistrubEnd)
            {
                // now is do not distrub time, so no warning sound
                return;
            }
